...
export function useInvoiceCreators(
  ...
  const createInvoiceFromTimesheets = useCallback((
    ...
    const { includeLineItems = true, applyTax = false, taxRate = 0.20,
            paymentTermsDays = 30, roundingPrecision = 2 } = options
    const lineItems: InvoiceLineItem[] = includeLineItems
      ? timesheets.map(ts => ({
          ...
          quantity: ts.hours || 0, rate: ts.rate || 0,
          amount: ts.amount || 0, timesheetId: ts.id
        }))
      : []
    ...
    return {
      id: `INV-${Date.now()}`, invoiceNumber: generateInvoiceNumber(), clientName,
      issueDate: issueDate.toISOString().split('T')[0],
      dueDate: dueDate.toISOString().split('T')[0],
      amount: total, status: 'draft', currency: 'GBP', lineItems, type: 'timesheet',
      paymentTerms: `Payment due within ${paymentTermsDays} days`
    }
  }, [generateInvoiceNumber])

  const createPlacementInvoice = useCallback((
    candidateName: string, clientName: string,
    salary: number, feePercentage: number,
    ...
    return {
      id: `INV-${Date.now()}`, invoiceNumber: generateInvoiceNumber('PLACE'), clientName,
      issueDate: issueDate.toISOString().split('T')[0],
      dueDate: dueDate.toISOString().split('T')[0],
      amount, status: 'draft', currency: 'GBP', type: 'permanent-placement',
      placementDetails: {
        candidateName, position: 'Position',
        startDate: issueDate.toISOString().split('T')[0],
        salary, feePercentage, guaranteePeriod: 90
      },
      paymentTerms: `Payment due within ${terms} days`
    }
  }, [generateInvoiceNumber])

  const createCreditNote = useCallback((
    originalInvoice: Invoice, creditAmount: number, reason: string
  ): Invoice => ({
    id: `CREDIT-${Date.now()}`,
