...
import {
  FileText, Plus, MagnifyingGlass, CheckCircle, Warning, CurrencyDollar, TrendUp, Download
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{vm.metrics.activeCount}</div>
            <p className="text-sm text-muted-foreground mt-1">
              {vm.metrics.remainingValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} remaining
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold font-mono">
              {vm.metrics.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
            <p className="text-sm text-muted-foreground mt-1">
              Across {vm.purchaseOrders.length} PO{vm.purchaseOrders.length !== 1 ? 's' : ''}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{vm.metrics.averageUtilization.toFixed(1)}%</div>
            <p className="text-sm text-muted-foreground mt-1">Average across active POs</p>
          </CardContent>
        </Card>
        <Card className={vm.metrics.expiringSoonCount > 0 ? 'border-warning/50' : ''}>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
          ...
          <CardContent>
            <div className="text-3xl font-bold">{vm.metrics.expiringSoonCount + vm.metrics.expiredCount}</div>
            <p className="text-sm text-muted-foreground mt-1">
              {vm.metrics.expiringSoonCount} expiring, {vm.metrics.expiredCount} expired
            </p>
          </CardContent>
        </Card>
      </div>
